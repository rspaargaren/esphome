 ...
 protected:
  ...
  bool read_message() {
    ...
    if (available()) {  // Check to be sure
      ...
      while (!footerfound &&
             currentMillis - previousMillis < 5000) {  // Loop while there's no footer found with a maximum of 5 seconds
        currentMillis = millis();
  ...
  void publish_sensors(MyData data) {
    ...
    if (data.energy_delivered_tariff2_present)
      s_energy_delivered_tariff2->publish_state(data.energy_delivered_tariff2);
    if (data.energy_returned_tariff1_present)
      s_energy_returned_tariff1->publish_state(data.energy_returned_tariff1);
    if (data.energy_returned_tariff2_present)
      s_energy_returned_tariff2->publish_state(data.energy_returned_tariff2);
    if (data.power_delivered_present)
      s_power_delivered->publish_state(data.power_delivered);
    if (data.power_returned_present)
      s_power_returned->publish_state(data.power_returned);
    if (data.voltage_l1_present)
      s_voltage_l1->publish_state(data.voltage_l1);
    if (data.voltage_l2_present)
      s_voltage_l2->publish_state(data.voltage_l2);
    if (data.voltage_l3_present)
      s_voltage_l3->publish_state(data.voltage_l3);
    if (data.current_l1_present)
      s_current_l1->publish_state(data.current_l1);
    if (data.current_l2_present)
      s_current_l2->publish_state(data.current_l2);
    if (data.current_l3_present)
      s_current_l3->publish_state(data.current_l3);
    if (data.power_delivered_l1_present)
      s_power_delivered_l1->publish_state(data.power_delivered_l1);
    if (data.power_delivered_l2_present)
      s_power_delivered_l2->publish_state(data.power_delivered_l2);
    if (data.power_delivered_l3_present)
      s_power_delivered_l3->publish_state(data.power_delivered_l3);
    if (data.power_returned_l1_present)
      s_power_returned_l1->publish_state(data.power_returned_l1);
    if (data.power_returned_l2_present)
      s_power_returned_l2->publish_state(data.power_returned_l2);
    if (data.power_returned_l3_present)
      s_power_returned_l3->publish_state(data.power_returned_l3);
    if (data.gas_device_type_present)
      s_gas_device_type->publish_state(data.gas_device_type);
    if (data.gas_valve_position_present)
      s_gas_valve_position->publish_state(data.gas_valve_position);
    if (data.gas_delivered_present)
      s_gas_delivered->publish_state(data.gas_delivered);
  };
